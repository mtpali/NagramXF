...
//        }
        ...
        if (messageEditText != null && messageEditText.getText().length() > 0) {
            ...
            options.add(R.drawable.ic_translate, getString(R.string.TranslateMessage), () -> {
                translateComment(Translator.getInputTranslateLangLocaleForChat(chatId));
                if (messageSendPreview != null) {
                    messageSendPreview.dismiss(false);
                    messageSendPreview = null;
...
//        }
    ...
    protected void createEmojiView() {
        ...
        emojiView.setDelegate(new EmojiView.EmojiViewDelegate() {
            ...
            @Override
            public void onSavedTextSelected(String text) {
                final EditTextBoldCursor editText = mOverrideEditTextView != null
                        ? mOverrideEditTextView : ChatActivityEnterView.this.messageEditText;
                if (editText == null || TextUtils.isEmpty(text)) return;
                int selectionStart = Math.max(0, editText.getSelectionStart());
                int selectionEnd = Math.max(0, editText.getSelectionEnd());
