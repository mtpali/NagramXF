...
cdef set special_attrs = set(dir(type) +                        # Special Python attributes
                             ["_chaquopy_j_klass",              # Chaquopy class attributes
                              "_chaquopy_j_access_all",         # Instance modifier flags
                              "_chaquopy_j_use_getter_setter",  #
                              "_chaquopy_reflector",            #
...
class JavaClass(type):
    ...
    def __getattribute__(cls, str name):
        _timing_active = (_chaquopy_should_timing_log() and
                          (name not in special_attrs))
        if _timing_active:
            _timing_started_ms = _chaquopy_timing_now_ms()
        try:
            if name.startswith("_"):
                return type.__getattribute__(cls, name)
            if j_modifiers_bootstrap_ready:
                ...
                if name not in special_attrs:
                    if not _chaquopy_get_class_access_all(cls):
                        if name in type_dict(cls):
                            return type.__getattribute__(cls, name)
                        if _chaquopy_should_use_direct_reflection_fast_path(
                                name, static_only=True):
                            if reflect_member(cls, name):
                                return type.__getattribute__(cls, name)
                    cached_plan = _chaquopy_get_cached_getattr_plan(
                        cls, name, False,
                        use_getter_setter=_chaquopy_get_class_use_getter_setter(cls),
                        static_only=True)
                    if cached_plan is _J_PLAN_FALLBACK and name in type_dict(cls):
                        return type.__getattribute__(cls, name)
                    result = _chaquopy_custom_class_getattr(cls, name)
                    if result is not _J_NO_MEMBER:
                        return result
            if name not in special_attrs and name not in type_dict(cls):
    ...
    def __setattr__(cls, str name, value):
        ...
        try:
            if name.startswith("_chaquopy"):
                ...
                return
            if j_modifiers_bootstrap_ready and \
               _chaquopy_try_custom_class_setattr(cls, name, value):
                return
            if name not in special_attrs and name not in type_dict(cls):
                reflect_member(cls, name)
            member = type_lookup(cls, name)
            if isinstance(member, JavaMember):
                member.set(None, value)
            else:
                type.__setattr__(cls, name, value)
        finally:
            if _timing_active:
    ...
    class JavaObject(metaclass=JavaClass):
        ...
        def __getattribute__(self, str name):
            ...
            try:
                ...
                if _chaquopy_is_native_exception_attr(self, name):
                    return object.__getattribute__(self, name)
                if j_modifiers_bootstrap_ready and _chaquopy_has_java_state(self):
                    if _chaquopy_maybe_modifier_name(name):
                        modifier_result = _chaquopy_handle_instance_modifier(self, name)
                        if modifier_result is not _J_NO_MEMBER:
                            return modifier_result
                    if name not in special_attrs:
                        if _chaquopy_is_known_python_attr(self, name):
                            return object.__getattribute__(self, name)
                        if not _chaquopy_get_access_all(self):
                            if name in type_dict(cls):
                                return object.__getattribute__(self, name)
                            if _chaquopy_should_use_direct_reflection_fast_path(name):
                                if reflect_member(cls, name):
                                    return object.__getattribute__(self, name)
                        result = _chaquopy_custom_getattr(self, cls, name)
                        if result is not _J_NO_MEMBER:
                            return result
                        if name not in type_dict(cls):
                            reflect_member(cls, name)
                return object.__getattribute__(self, name)
            finally:
                if _timing_active:
        ...
        def __setattr__(self, str name, value):
            ...
            if _timing_active:
                _timing_started_ms = _chaquopy_timing_now_ms()
            try:
                if name.startswith("_") and not name.startswith("_chaquopy"):
                    # Private attributes stay on the instance.
                    object.__setattr__(self, name, value)
                    if name in self.__dict__:
                        _chaquopy_register_python_attr(self, name)
                    return
                if name.startswith("_chaquopy"):
                    object.__setattr__(self, name, value)
                    return
                ...
                if j_modifiers_bootstrap_ready and _chaquopy_has_java_state(self):
                    if _chaquopy_is_known_python_attr(self, name):
                        object.__setattr__(self, name, value)
                        return
                    if _chaquopy_try_custom_setattr(self, name, value):
                        return
                ...
                if (name in self.__dict__) and \
                   not (name.startswith("_chaquopy") or name in special_attrs or
                        _chaquopy_is_native_exception_attr(self, name)) and \
...
cdef _chaquopy_register_python_attr(obj, name):
    cls = type(obj)
...
# Declare only the methods needed to complete the bootstrap process.
    ...
    try:
        ...
        bootstrap_method(Reflector, "dirAll", "()[Ljava/lang/String;")
    ...
    try:
        ...
        bootstrap_method(Class, "getInterfaces", '()[Ljava/lang/Class;')
...
cdef _chaquopy_collect_methods(cls, str name, bint access_all):
    """All Java methods of the given name across the reflectable MRO."""
    jms = []
    for c in cls.__mro__:
        if not _chaquopy_is_reflectable_java_class(c):
            continue
        reflector = get_reflector(c)
        members = reflector.getMethodsAll(name) if access_all else reflector.getMethods(name)
        for m in (members or []):
            jms.append(JavaMethod(c, name, m))
    return jms
...
cdef _chaquopy_lookup_exact_nonfield_member(cls, str name, bint access_all=False,
    ...
    cache_key = ("exact_nonfield", name, int(access_all),
                 _chaquopy_static_filter_key(static_only))
    cached = _chaquopy_lookup_cache_get(cls, cache_key)
    if cached is not _J_CACHE_MISS:
        return cached
    if access_all:
        jms = _chaquopy_filter_java_method_list(cls, _chaquopy_collect_methods(cls, name, True))
        if len(jms) == 1:
            return _chaquopy_lookup_cache_set(cls, cache_key, jms[0])
        if len(jms) > 1:
            return _chaquopy_lookup_cache_set(cls, cache_key,
                                              JavaMultipleMethod(cls, name, jms))
        nested = _chaquopy_find_declared_nested_class(cls, name, need_all=True)
        return _chaquopy_lookup_cache_set(
            cls, cache_key, nested if nested is not None else _J_NO_MEMBER)
    member = cls.__dict__.get(name)
    if member is None and _chaquopy_is_reflectable_java_class(cls):
        # reflect_member backfills cls.__dict__, so misses stop repeating.
        member = reflect_member(cls, name)
    member = _chaquopy_filter_method_member(cls, member)
    if member is None:
        return _chaquopy_lookup_cache_set(cls, cache_key, _J_NO_MEMBER)
    return _chaquopy_lookup_cache_set(cls, cache_key, member)
...
cdef _chaquopy_custom_class_getattr(cls, str name):
    ...
    kind, member = plan
    if kind == "getter":
        return member.get(None, cls)()
    return _chaquopy_bind_dynamic_member(member, None, cls)


cdef bint _chaquopy_try_custom_setattr(obj, str name, value) except -1:
    if name.startswith("_chaquopy"):
        return 0
    cdef bint use_gs = _chaquopy_get_use_getter_setter(obj)
    cdef bint access_all = _chaquopy_get_access_all(obj)
...
cdef class JavaMember(object):
    ...
    # Plan machinery calls members through .get/.set.
...
cdef class JavaPropertyAlias(JavaMember):
    ...
    cdef public object _getter
    cdef public object _setter
    cdef public object _fallback_member
...
cdef class JavaSimpleMember(JavaMember):
    cdef readonly bint is_static
    cdef readonly bint is_final
...
cdef class JavaMethod(JavaSimpleMember):
    ...
    cdef readonly bint is_constructor
    cdef readonly bint is_abstract
    cdef readonly bint is_varargs
