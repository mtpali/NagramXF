...
# Prevent the vendored elftools from pulling in the real pdb during bootstrap.
...
class AssetFinder:
    ...
    def extract_dir(self, zip_dir, recursive=True):
        ...
        extract_package = any(ep in (dotted_dir, "*") or dotted_dir.startswith(ep + ".")
...
PYTHON_SUFFIXES = tuple(suffix for suffix in LOADERS
                        if suffix != ".so")
